...
export function CampaignTable({ data, isLoading }: Props) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Kampagner hest</CardTitle>
      </CardHeader>
      <CardContent className="overflow-x-auto p-0">
        {isLoading ? (
          ...
          <p className="p-6 text-sm text-muted-foreground">No campaigns to display.</p>
        ) : (
              ...
              {sorted.map((row, i) => (
                ...
                >
                  <td className="px-4 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${PLATFORM_COLORS[row.platform] ?? ''}`}>
                      {row.platform}
                    </span>
                  </td>
                  <td className="max-w-[200px] truncate px-4 py-3 font-medium">{row.campaignName}</td>
                  {cols.map((c) => (
